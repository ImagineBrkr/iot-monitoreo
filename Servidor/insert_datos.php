...
for($i = 1; $i <= $cantidad; $i++) {
	$energia = rand(0, 14400);
	$valores = $feMedicion->format('Y-m-d-H-i-s');
	$valores = explode('-',$valores);
    $result = $mysqli->query("insert into medicion (laboratorio,medicion,fecha) values ('$lab', '$energia', '".$feMedicion->format('Y-m-d')."')");
 	$result = $mysqli->query("update medicion set fecha = DATE_ADD(DATE_ADD(fecha, INTERVAL ".$valores[3]." HOUR), INTERVAL ".$cantidadH." MINUTE) where laboratorio = '$lab' and fecha = '".$feMedicion->format('Y-m-d')."'");
